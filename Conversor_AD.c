...
// Função genérica para desenhar um retângulo com espaçamento ajustável (tracejado ou pontilhado)
...
void draw_border(uint8_t style)
{
    if (style == 0)
    {
        draw_pattern_rect(3, 3, 122, 60, 4); // Borda tracejada
    }
    else if (style == 1)
    {
        draw_pattern_rect(3, 3, 122, 60, 2); // Borda pontilhada
    }
    ssd1306_send_data(&display);
}
